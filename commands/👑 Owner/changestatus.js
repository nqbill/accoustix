    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Change Status', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/au-kddi/190/purple-heart_1f49c.png', 'https://discord.gg/homely')
        ...
        collector.on('collect', menu => {
          ...
          else menu?.reply({content: `<a:nooo:961036403075072071> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yesss:961036336813461534> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
          ...
          case "Status 1. Text":
            {
              var tempmsg = await message.reply({embeds: [new MessageEmbed()
                ...
                .setColor(es.color)
                .setDescription(`Example: \`${prefix}help | ${client.user.username.split(" ")[0]} | by: homely.dev\`
