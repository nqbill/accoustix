  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    if (user) {
      if (!user.bot) return interaction.reply({
        ephemeral: true,
        content: "<a:nooo:961036403075072071> You can't Invite a Normal user! **IT MUST BE A BOT**"
      })
      let button_public_invite = new MessageButton().setStyle('LINK').setLabel(handlemsg(client.la[ls].cmds.info.invite.buttons.public)).setURL("https://discord.com/api/oauth2/authorize?client_id=961022525385609216&permissions=8&scope=bot%20applications.commands")
