...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (!member.voice.channel)
      return interaction.reply({
        content: `<a:nooo:961036403075072071> **Please join a Voice Channel first!**`,
        ephemeral: true
      })
    ...
    if (player && member.voice.channel.id !== player.voiceChannel)
      return interaction.reply({
        content: `<a:nooo:961036403075072071> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
        ephemeral: true
      })
    if (interaction.isButton()) {
      if (!player || !player.queue || !player.queue.current) {
        return interaction.reply({
          content: "<a:nooo:961036403075072071> Nothing Playing yet",
          ephemeral: true
        })
      }
      ...
      if (player && interaction.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
        return interaction.reply({
          embeds: [new MessageEmbed()
            ...
            .setTitle(`<a:nooo:961036403075072071> **You are not a DJ and not the Song Requester!**`)
      ...
      case "Autoplay": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.get(`autoplay`) ? `<a:yesss:961036336813461534> **Enabled Autoplay**`: `<a:nooo:961036403075072071> **Disabled Autoplay**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Song": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.trackRepeat ? `<a:yesss:961036336813461534> **Enabled Song Loop**`: `<a:nooo:961036403075072071> **Disabled Song Loop**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Queue": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.queueRepeat ? `<a:yesss:961036336813461534> **Enabled Queue Loop**`: `<a:nooo:961036403075072071> **Disabled Queue Loop**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
  ...
  client.on("messageCreate", async message => {
    ...
    if (prefixRegex.test(message.content)) {
      ...
      if (cmd || cmd.length === 0) return // message.reply("<a:nooo:961036403075072071> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
      ...
      {
        return // message.reply("<a:nooo:961036403075072071> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
      }
    }
    ...
    if (!channel) return message.reply("<a:nooo:961036403075072071> **Please join a Voice Channel first!**").then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
    ...
    if (player && channel.id !== player.voiceChannel) return message.reply(`<a:nooo:961036403075072071> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
...
function generateQueueEmbed(client, guildId, leave) {
  ...
  var embeds = [
    ...
    .setImage("https://raw.githubusercontent.com/nqbill/Accoustix/main/src/img/accoustix_banner.png"),
    new MessageEmbed()
    .setColor(es.color)
    .setFooter(client.getFooter(es))
    .setImage(guild.banner ? guild.bannerURL({
      size: 4096
    }) : "https://raw.githubusercontent.com/nqbill/Accoustix/main/src/img/accoustix.jpg")
    .setTitle(`Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`)
    .setDescription(`> *I support  <:youtubeee:961034088200945715> Youtube, <:spotifyyy:961034035688255488> Spotify, <:soundclouddd:961034190948814898> Soundcloud and direct MP3 Links!*`)
  ]
